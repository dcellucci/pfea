...
def applyForce(force,reactions):
    ...
    loads.append({'node':node_frame_map[1][1][1][0],'DOF':0, 'value':weight/4})
    loads.append({'node':node_frame_map[1][2][1][0],'DOF':0, 'value':weight/4})
    loads.append({'node':node_frame_map[1][1][2][0],'DOF':0, 'value':weight/4})
    loads.append({'node':node_frame_map[1][2][2][0],'DOF':0, 'value':weight/4})

    index = np.array([[1,1,1],[1,1,2],[1,2,1],[1,2,2]])
...
def writeForceCSV(reactions,force,goal,index):
    np.savetxt('/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x2/Force2x2x2ActDist'+str(goal)+'Index'+str(index)+'.csv', np.concatenate((reactions,force*np.ones((reactions.size/3,1))), axis=1), delimiter=',')
...
for goal in actuationRange:
    ...
    bottomDisp = np.zeros((4,3))
    reactForce = np.zeros((4,3))
    ...
    while converge>0.01:
        ...
        converge = (goal-res_displace[25][0])/goal
        #print(converge)
        ...
        cofFrict = np.zeros((len(minIndex[0]),1))
    ...
    writeForceCSV(reactions,force,goal,index)
    prevNodes = np.zeros((36,6))
    prevNodes[:,0:3] = out_nodes
    prevNodes = prevNodes+res_displace
    pfea.util.pfeautil.writeCSV(nodes,res_displace,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x2/2x2x2ActDist'+str(goal)+'Index'+str(index)+'.csv')
    prevGoal = goal
    index = index + 1
    
#print(res_displace[25][0])
#pfea.util.pfeautil.plotLattice(nodes,frames,res_displace,1)
pfea.solver.write_K(out_nodes,out_frames,global_args,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x2/KCube.txt')
pfea.solver.write_M(out_nodes,out_frames,global_args,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x2/MCube.txt')
np.savetxt('2x2x2Frames.csv', out_frames[0][0], delimiter=',')
