...
def writeForceCSV(reactions,force,goal,index):
    np.savetxt('/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x4/Force2x2x4ActDist'+str(goal)+'Index'+str(index)+'.csv', np.concatenate((reactions,force*np.ones((reactions.size/3,1))), axis=1), delimiter=',')
...
for goal in actuationRange:
    ...
    while converge>0.01:
        ...
        converge = (goal-res_displace[46][0])/goal
        #print(converge)
        #print(res_displace[46][0])
    ...
    pfea.util.pfeautil.writeCSV(nodes,res_displace,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x4/2x2x4ActDist'+str(goal)+'Index'+str(index)+'.csv')
    prevGoal = goal
    index = index + 1
    
#print(res_displace[25][0])
pfea.solver.write_K(out_nodes,out_frames,global_args,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x4/K2x2x4.txt')
pfea.solver.write_M(out_nodes,out_frames,global_args,'/home/nick/Documents/FERVORSimulationResults/CorrectFriction/2x2x4/M2x2x4.txt')
np.savetxt('2x2x4Frames.csv', out_frames[0][0], delimiter=',')
