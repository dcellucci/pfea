...
for x in range(1,len(mat_matrix)-1):
	for y in range(1,len(mat_matrix[0])-1):
		#The bottom-most nodes are constrained to neither translate nor
		#rotate
		if mat_matrix[x][y][1] == 1.0:# and abs(1.0*(y+0.5)-len(mat_matrix[0])/2.0)<=6.0*subdiv:
			#print(node_frame_map[x][y][1][2])
		...
		#The top most nodes are assigned a z-axis load
		if y > len(mat_matrix[0])/2.0-2 and y < len(mat_matrix[0])/2.0+1 and mat_matrix[x][y][len(mat_matrix[0][0])-2]==1.0:
			#print(node_frame_map[x][y][len(mat_matrix[0][0])-2][5])
			loads.append({'node':node_frame_map[x][y][len(mat_matrix[0][0])-2][5],'DOF':2, 'value':-benjamin})
...
#Global Arguments 
global_args = {'frame3dd_filename': "br_s{0}_mt".format(subdiv), 'length_scaling':1,"using_Frame3dd":False,"debug_plot":True}
